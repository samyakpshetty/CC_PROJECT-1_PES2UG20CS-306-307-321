		...
		go func(peerId int) {
			...
			if err := this.server.SendRPCCallTo(peerId, "RaftNode.RequestVote", args, &reply); err == nil {
				...
				if reply.Term > this.currentTerm {
					...
					if this.state == "Candidate" && reply.VoteGranted {
						votesReceived++
						if votesReceived > len(this.peersIds)/2 {
							this.startLeader()
						}
					} else if reply.VoteGranted {
						this.write_log("Already voted for %d in this term", reply.CandidateId)
					} else {
						this.write_log("Vote not granted by %d ", reply.CandidateId)
					}
				} else {
					this.write_log("Ignoring the reply with the term %d (current term : %d ) ", reply.Term, this.currentTerm)
				}
			}
		}(peerId)
	}
