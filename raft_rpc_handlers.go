...
func (this *RaftNode) HandleRequestVote(args RequestVoteArgs, reply *RequestVoteReply) error {
	...
	//-------------------------------------------------------------------------------------------/
	if (this.votedFor == -1 || this.votedFor == args.CandidateId) && args.Term >= this.currentTerm && (args.LastLogTerm > nodeLastLogTerm || (args.LastLogTerm == nodeLastLogTerm && args.LastLogIndex >= nodeLastLogIndex)) {
